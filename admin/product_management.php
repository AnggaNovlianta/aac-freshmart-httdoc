...
$stmt_total->execute();
$total_filtered_products = $stmt_total->get_result()->fetch_row()[0];
$total_pages = ceil($total_filtered_products / $limit);
$stmt_total->close();

// --- 4. QUERY UTAMA UNTUK MENGAMBIL PRODUK ---
$products_query = "
...
$current_params = $params; $current_types = $types;
$current_params[] = $limit; $current_params[] = $offset; $current_types .= 'ii';
...
<head>
    ...
    <style> 
        ...
        .product-thumbnail { width: 50px; height: 50px; object-fit: cover; } 
        .stat-card i { opacity: 0.5; } 
...
<div class="modal fade" id="productModal" tabindex="-1" aria-labelledby="productModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header"><h5 class="modal-title" id="productModalLabel">Form Produk</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div>
      <form id="productForm" action="../_process/process_product.php" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-body">
            ...
            <div class="row"><div class="col-md-8 mb-3"><label for="form-name" class="form-label">Nama Produk</label><input type="text" class="form-control" id="form-name" name="name" required></div><div class="col-md-4 mb-3"><label for="form-sku" class="form-label">SKU</label><input type="text" class="form-control" id="form-sku" name="sku" readonly></div></div>
            <div class="row"><div class="col-md-6 mb-3"><label for="form-category" class="form-label">Kategori</label><select class="form-select" id="form-category" name="category_id" required><option value="" selected disabled>-- Pilih --</option><?php if ($categories_result->num_rows > 0) { $categories_result->data_seek(0); while($cat = $categories_result->fetch_assoc()) { echo "<option value='{$cat['id']}'>{$cat['name']}</option>"; } } ?></select></div><div class="col-md-6 mb-3"><label for="form-supplier" class="form-label">Supplier</label><select class="form-select" id="form-supplier" name="supplier_id"><option value="">-- Pilih --</option><?php if ($suppliers_result->num_rows > 0) { $suppliers_result->data_seek(0); while($sup = $suppliers_result->fetch_assoc()) { echo "<option value='{$sup['id']}'>{$sup['name']}</option>"; } } ?></select></div></div>
            ...
            <div class="row"><div class="col-md-4 mb-3"><label for="form-cost-price" class="form-label">Harga Modal (Rp)</label><input type="number" class="form-control" id="form-cost-price" name="cost_price" value="0"></div><div class="col-md-4 mb-3"><label for="form-selling-price" class="form-label">Harga Jual (Rp)</label><input type="number" class="form-control" id="form-selling-price" name="selling_price" value="0" required></div><div class="col-md-4 mb-3"><label for="form-stock" class="form-label">Jumlah Stok</label><input type="number" class="form-control" id="form-stock" name="stock" value="0" required></div></div>
            <div class="row"><div class="col-md-6 mb-3"><label for="form-low-stock" class="form-label">Ambang Stok Rendah</label><input type="number" class="form-control" id="form-low-stock" name="low_stock_threshold" value="10"></div><div class="col-md-6 mb-3"><label for="form-weight" class="form-label">Berat (Kg)</label><input type="number" class="form-control" id="form-weight" name="weight_kg" step="0.01" value="0"></div></div>
...
<form id="deleteForm" action="../_process/process_product.php" method="POST" style="display: none;"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" id="delete-id"></form>

<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
<script>
function confirmDelete(id) { if (confirm("Yakin ingin menghapus produk ini? Aksi ini tidak bisa dibatalkan.")) { document.getElementById('delete-id').value = id; document.getElementById('deleteForm').submit(); } }
