...
$action = $_POST['action'] ?? '';
$redirect_url = BASE_URL . "admin/user_menu_management.php"; // Redirect ke halaman manajemen yang benar
...
if ($action == 'add') {
    ...
    $stmt = $conn->prepare("INSERT INTO user_menus (title, description, icon_class, url, order_number, is_active) VALUES (?, ?, ?, ?, ?, ?)");
...
if ($action == 'edit') {
    ...
    $stmt = $conn->prepare("UPDATE user_menus SET title=?, description=?, icon_class=?, url=?, order_number=?, is_active=? WHERE id=?");
...
if ($action == 'delete') {
    ...
    $stmt = $conn->prepare("DELETE FROM user_menus WHERE id = ?");
