...
$action = $_POST['action'] ?? '';
// PENTING: Ubah URL redirect ke halaman manajemen menu keuangan
$redirect_url = BASE_URL . "admin/menu_keuangan_management.php";
...
if ($action == 'add') {
    ...
    // PENTING: Query ke tabel keuangan_menus
    $stmt = $conn->prepare("INSERT INTO keuangan_menus (title, description, icon_class, url, order_number, is_active) VALUES (?, ?, ?, ?, ?, ?)");
...
if ($action == 'edit') {
    ...
    // PENTING: Query ke tabel keuangan_menus
    $stmt = $conn->prepare("UPDATE keuangan_menus SET title=?, description=?, icon_class=?, url=?, order_number=?, is_active=? WHERE id=?");
...
if ($action == 'delete') {
    ...
    // PENTING: Query ke tabel keuangan_menus
    $stmt = $conn->prepare("DELETE FROM keuangan_menus WHERE id = ?");
